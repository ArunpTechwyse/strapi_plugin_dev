...
function ProductCheckbox({ value, checkboxID, callback, disabled }) {
...
function ProductInput({ value, onChange }) {
  return (
    <TextInput
      type="text"
        ...
        <Tbody>
          {productData.map((product) => {
            ...
            return (
              <Tr key={product.id}>
                <Td>
                  <Typography textColor="neutral800">{product.id}</Typography>
                </Td>
                ...
                <Td>
                  {isEdit ? (
                    <Flex style={{ justifyContent: "end" }}>
                      <Button
                        onClick={() => editProduct(product.id, { name: inputValue })}
                      >
                        Save
                      </Button>
                    </Flex>
